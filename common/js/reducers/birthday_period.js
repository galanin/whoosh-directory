  ...
  switch (action.type) {
    ...
    case LOCATION_CHANGE:
      if (action.payload.action === 'POP') {
        const birthday_period_packed = getNewUrlParam(
          action.payload,
          URL_PARAM_BIRTHDAY_PERIOD
        );
        if (birthday_period_packed) {
          const birthday_period = unpackBirthdayPeriod(birthday_period_packed);
          if (birthday_period.day_offset_start) {
            birthday_period.day_scroll_to = birthday_period.day_offset_start;
          }
