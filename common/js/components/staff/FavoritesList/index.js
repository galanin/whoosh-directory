      ...
      div({ className: 'favorites-list-container' },
        div({ className: 'favorites-list__header' },
          ...
          span({ className: 'favorites-list__header-buttons' },
            span({ className: 'favorites-list__header-button favorites-list__header-button-employments', onClick: this.showEmployments.bind(this) },
              'Сотрудники'),
            span({ className: 'favorites-list__header-button favorites-list__header-button-units', onClick: this.showUnits.bind(this) },
              'Оргструктура')
          )
        ),
        ...
        (() => { switch (this.props.show) {
          case FAVORITE_PEOPLE:
            if (isArray(this.props.people) && !isEmpty(this.props.people)) {
              return div({ className: 'favorite-list__employments' },
                (() => {
                  const result = [];
                  for (let favorite_people of Array.from(this.props.people)) {
                    if (favorite_people.employment_id != null) {
                      result.push(someone({ key: favorite_people.employment_id, employment_id: favorite_people.employment_id, className: 'list-item shadow' }));
                    } else if (favorite_people.contact_id != null) {
                      result.push(someone({ key: favorite_people.contact_id, contact_id: favorite_people.contact_id, className: 'list-item shadow' }));
                    } else {
          ...
          case FAVORITE_UNITS:
            if (isArray(this.props.units) && !isEmpty(this.props.units)) {
              return div({ className: 'favorite-list__units' },
                Array.from(this.props.units).map((favorite_unit) =>
