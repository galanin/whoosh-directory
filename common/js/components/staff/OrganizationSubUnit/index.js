...
class OrganizationSubUnit extends React.Component {
  ...
  render() {
    if (this.props.unit_data == null) { return ''; }
    ...
    return div({ className: sub_unit_class_names },
      div({ className: 'sub-unit__head', onClick: this.onExpandCollapseClick.bind(this) },
        div({ className: 'sub-unit__button' },
          svg({ className: 'sub-unit__triangle', svg: Triangle })),
        div({ className: 'sub-unit__title' },
          this.props.unit_data.list_title)
      ),
      div({ className: 'sub-unit__content' },

        this.props.is_expanded && isArray(this.props.unit_data != null ? this.props.unit_data.employ_ids : undefined) ?
          div({ className: 'sub-unit__employees' },
            Array.from(this.props.unit_data.employ_ids).map((employment_id) =>
              someone({key: employment_id, employment_id, hide: { unit: true }, className: 'list-item shadow'}))
          ) : undefined,

        this.props.is_expanded && isArray(this.props.unit_data != null ? this.props.unit_data.child_ids : undefined) ?
          div({ className: 'sub-unit__sub-units' },
            Array.from(this.props.unit_data.child_ids).map((sub_unit_id) =>
              sub_unit({ key: 'sub-unit-' + sub_unit_id, unit_id: sub_unit_id }))) : undefined));
  }
}


const ConnectedOrganizationSubUnit = connect(mapStateToProps, mapDispatchToProps)(OrganizationSubUnit);
