...
class Node extends React.Component {
  ...
  hasChildren() {
    return +(this.props.node.c != null ? this.props.node.c.length : undefined) > 0;
  }
  ...
  render() {
    ...
    return div({ className: node_class_name },
      has_children ?
        div({ className: 'node__button', onClick: this.onExpandCollapseClick.bind(this) },
      ...
      div({ className: 'node__content' },
        scroll_element({ className: title_class_name, onClick: this.onNodeClick.bind(this), name: `node-${this.props.node_id}` },
          this.props.node.t),
        has_children ?
          div({ className: 'node__children' },
            Array.from(this.props.node.c).map((child_id) =>
              node({ key: child_id, node_id: child_id }))) : undefined));
  }
}
